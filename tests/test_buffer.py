...
class TestGeometryBuffer:
    ...
    def test_geometry_buffer_point(self):
        ...
        buffered = geom.buffer(1.0, quad_segs=16)
        assert buffered is not None
        assert buffered.geom_type == "Polygon"
    ...
    def test_geometry_buffer_with_custom_styles(self):
        ...
        buffered = geom.buffer(
            2.0, quad_segs=16, cap_style=3, join_style=2, mitre_limit=3.0
        )
        assert buffered is not None
        assert buffered.geom_type == "Polygon"
        assert buffered.area > 0
...
class TestPointBuffer:
    ...
    def test_point_buffer_creates_polygon(self):
        ...
        p = Point(0, 0)
        buffered = p.buffer(1.0, quad_segs=16)
        assert buffered.geom_type == "Polygon"
        # Buffer should approximate circle: π * r²
        expected_area = math.pi * 1.0 * 1.0
        assert abs(buffered.area - expected_area) < 0.1

    def test_point_buffer_with_quad_segs(self):
        """Test Point buffer with different quad_segs"""
        p = Point(5.5, 3.2)
        buffered = p.buffer(2.0, quad_segs=16)
        assert buffered.geom_type == "Polygon"
...
class TestLineStringBuffer:
    ...
    def test_linestring_buffer_creates_polygon(self):
        ...
        buffered = line.buffer(1.0, quad_segs=8)
        assert buffered.geom_type == "Polygon"
    ...
    def test_linestring_buffer_with_cap_styles(self):
        ...
        # Round cap (default)
        buffered1 = line.buffer(1.0, cap_style=1, quad_segs=16)
        # Square cap
...
class TestPolygonBuffer:
    ...
    def test_polygon_buffer_outward(self):
        ...
        buffered = poly.buffer(1.0, quad_segs=8)
        assert buffered.geom_type == "Polygon"
        # Positive buffer should increase area
        assert buffered.area > original_area
        # Original should be contained in buffered
        assert buffered.contains(poly.as_geometry())
    ...
    def test_polygon_buffer_inward(self):
        ...
        assert poly.as_geometry().contains(buffered)
...
class TestRingBuffer:
    ...
    def test_ring_buffer_creates_polygon(self):
        ...
        ring = Ring([(0, 0), (5, 0), (5, 5), (0, 5), (0, 0)])
        buffered = ring.buffer(1.0, quad_segs=8)
        assert buffered.geom_type == "Polygon"
    ...
    def test_ring_buffer_with_join_styles(self):
        ...
        ring = Ring([(0, 0), (5, 0), (5, 5), (0, 5), (0, 0)])
        # Round join (default)
        buffered1 = ring.buffer(1.0, join_style=1, quad_segs=16)
        # Bevel join
...
class TestBufferParameters:
    ...
    def test_buffer_quad_segs_values(self):
        """Test buffer with various quad_segs values"""
        geom = Geometry("POINT(0 0)", fmt="wkt")
        areas = []
        for quad_segs in [4, 8, 16, 32]:
            buffered = geom.buffer(1.0, quad_segs=quad_segs)
            assert buffered.geom_type == "Polygon"
            areas.append(buffered.area)
        # Higher quad_segs should produce area closer to π*r²
        expected = math.pi * 1.0 * 1.0
        # Higher quad_segs should be more accurate
    ...
    def test_buffer_cap_styles(self):
        ...
        for cap_style in [1, 2, 3]:
            buffered = geom.buffer(1.0, cap_style=cap_style, quad_segs=16)
            assert buffered.geom_type == "Polygon"
            assert buffered.area > 0
            areas.append(buffered.area)
...
class TestBufferErrorCases:
    ...
    def test_buffer_very_large_negative_distance(self):
        ...
        buffered = geom.buffer(-100.0, quad_segs=8)
    ...
    def test_buffer_negative_quad_segs(self):
        """Test buffer with negative quad_segs value"""
        geom = Geometry("POINT(0 0)", fmt="wkt")
        # Negative quad_segs should fail or be handled by GEOS
        with pytest.raises(ValueError):
            geom.buffer(1.0, quad_segs=-1)

    def test_buffer_zero_quad_segs(self):
        """Test buffer with zero quad_segs value"""
        geom = Geometry("POINT(0 0)", fmt="wkt")
        # Zero quad_segs should fail as it's invalid
        with pytest.raises(ValueError):
            geom.buffer(1.0, quad_segs=0)

    def test_buffer_very_small_distance(self):
        """Test buffer with very small distance value"""
        geom = Geometry("POINT(0 0)", fmt="wkt")
        # Very small buffer should still work
        buffered = geom.buffer(0.0001, quad_segs=8)
        assert buffered.geom_type == "Polygon"
    ...
    def test_buffer_large_positive_distance(self):
        ...
        buffered = geom.buffer(1000.0, quad_segs=8)
        assert buffered.geom_type == "Polygon"
    ...
    def test_buffer_mitre_limit_equals_zero(self):
        """Test buffer with negative mitre_limit"""
        geom = Geometry("LINESTRING(0 0, 10 0)", fmt="wkt")
        # Negative mitre_limit might be invalid
        with pytest.raises(ValueError):
            geom.buffer(1.0, join_style=2, mitre_limit=0.0)
